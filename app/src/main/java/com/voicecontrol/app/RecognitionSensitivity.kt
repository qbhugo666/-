 ...
 * 高灵敏度的最坏情况是误识别变多，由既有三道保险兜底——媒体音量 80% 硬帽 / 同命令 0.8s 冷却（v0.55.4）/
...
object RecognitionSensitivity {
    ...
    /** VAD 最短开口时长（秒）：v0.57.8 从 0.25 压到 0.15——撤预滚后用户实测起音被吞
     *  （「重复三次」→「负三次」「两次」，2026-09-22 使用记录实锤），门卫攒证据时间缩短吃得更少；
     *  回退条件：噪音误触发分段变多（误识别率上升）→ 改回 0.2/0.25。格 10 远场 0.2→0.15 同步 */
    fun minSpeech(level: Int): Float = 0.15f
}
